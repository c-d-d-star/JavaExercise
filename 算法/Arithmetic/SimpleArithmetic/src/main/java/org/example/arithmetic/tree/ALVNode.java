...
class MyAVLTree<E extends Comparable<E>> {
...
把当前节点的值换为左子节点的值：value=leit.vailue
...
把当前节点的右子树设置为新节点：right=newNode
    ...
    public static void main(String[] args) {
        MyAVLTree<Integer> searchTree = new MyAVLTree<Integer>();
        for (int i = 0; i < 10; i++) {
            searchTree.insert(i);
        }
        System.out.println("```");
        MyAVLTree<Integer> avlTree = new MyAVLTree<Integer>();
        for (int i = 0; i < 10; i++) {
            avlTree.insert(i);
